...
def get_gemini_negotiation_response(request, product, user_message, chat_history):
    ...
    UPDATED: Now handles language session locking to maintain conversation flow.
    """
    product_price = product.price

    # 0. Detect and Lock Language Preference
    lang_key = f'negotiation_language_{product.slug}'
    session_language = request.session.get(lang_key)
    is_luganda_session = False
    
    # 0a. Check for persistent session language preference
    if session_language == 'luganda':
        is_luganda_session = True
    
    # 0b. If no session language, try to detect it from the current message
    elif session_language is None:
        if is_luganda(user_message):
            is_luganda_session = True
            request.session[lang_key] = 'luganda' # Lock language for the rest of the negotiation
        else:
            request.session[lang_key] = 'english' # Lock language for the rest of the negotiation
...
def get_ai_response(request, product, user_message, chat_history):
    # This is the entry point called by ai_negotiation_view, now passing the request
...
def ai_negotiation_view(request, slug):
    ...
    if chat_history is None:
        # Initial greeting is now multilingual
        initial_greeting = f"Hello! I'm the AI Negotiator, and I'm ready to find you a great price. The original price for **{product.name}** is UGX {product.price:,.0f}. What is your first offer? (I also speak Luganda!)"
        chat_history = [
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        # 2. Get AI response and add it to history (Passing request object now)
...
def accept_negotiated_price(request, slug):
    ...
    if product.negotiated_price and product.negotiated_price <= product.price:
        ...
        # Clear chat history and negotiation language for this product
        if f'chat_history_{slug}' in request.session:
            del request.session[f'chat_history_{slug}']
        if f'negotiation_language_{slug}' in request.session:
            del request.session[f'negotiation_language_{slug}']
