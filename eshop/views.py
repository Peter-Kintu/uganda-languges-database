...
def get_gemini_negotiation_response(product, user_message, chat_history):
    ...
    # 2. Parse user offer - Robust Parsing
    offer = None
    raw_offer_text = None 
    ...
    if offer_match:
        try:
            ...
            # Heuristic for missing '000' (UGX 250,000 -> user types 200)
    ...
    if offer is None:
        ...
        # Default fallback
        # This is where a real Gemini call would be used for conversational context
    ...
    if offer < VENDOR_MIN_ENGAGEMENT: 
        ...
        # Determine the price AI is currently defending (the last offered price, or the first offer price)
        if last_ai_offer >= product_price * Decimal('0.99'):
             ...
             # If between 90% and 98%, defend the current price
             display_floor = last_ai_offer
        
        else: # at 90%
             # If at final floor, defend the final floor
             display_floor = FINAL_FLOOR
        

        return f"I appreciate the offer of **{raw_offer_text}**, but it's too low for us to even consider. I can't let it go for less than **UGX {display_floor:,.0f}** to start the negotiation. Please make me a better offer."
